  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
            ...
            itemBuilder: (context, index) {
              ...
              return _RecipeCard(
                  ...
                  onTap: () => Navigator.push(
                      context,
                      MaterialPageRoute(builder: (_) => RecipeDetailPage(recipe: recipe, initialRecipe: recipe)),
                  ),
              );
            },
          );
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
            if (user == null) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(loc.user_not_authenticated)));
                return;
            }
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(recipe.name, style: GoogleFonts.recursive(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  Row(
                    children: [
                       const Icon(Icons.timer, size: 16, color: Colors.grey),
                       const SizedBox(width: 4),
                       Text("${recipe.prepTime} min", style: GoogleFonts.recursive(color: Colors.grey)),
                       const SizedBox(width: 16),
                        const Icon(Icons.local_fire_department, size: 16, color: Colors.grey),
                       const SizedBox(width: 4),
                       Text("${recipe.bakingTime} min", style: GoogleFonts.recursive(color: Colors.grey)),
                    ],
                  )
                ],
              ),
