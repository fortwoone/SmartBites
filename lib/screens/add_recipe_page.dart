...
class _AddRecipePageState extends State<AddRecipePage> {
      ...
      builder: (context) => SimpleDialog(
        ...
        children: [
          ...
          SimpleDialogOption(
            ...
            child: const Text('Ajouter via recherche'),
          ),
          SimpleDialogOption(
  ...
  Future<void> _saveRecipe() async {
    ...
    SnackBar(content: Text(_isEditing ? 'Recette mise à jour !' : 'Recette ajoutée !')),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_isEditing ? 'Modifier la recette' : 'Ajouter une recette'),
      ...
      body: Padding(
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              TextFormField(controller: _nameController, decoration: _inputDecoration('Nom de la recette'), validator: (val) => val == null || val.isEmpty ? 'Entrez un nom' : null),
              const SizedBox(height: 12),
              TextFormField(controller: _descriptionController, decoration: _inputDecoration('Description'), maxLines: 2),
              const SizedBox(height: 12),
              Row(
                children: [
                  Expanded(child: TextFormField(controller: _timePreparationController, decoration: _inputDecoration('Temps de préparation (min)'), keyboardType: TextInputType.number)),
                  const SizedBox(width: 12),
                  Expanded(child: TextFormField(controller: _timeBakingController, decoration: _inputDecoration('Temps de cuisson (min)'), keyboardType: TextInputType.number)),
                ],
              ),
              const SizedBox(height: 12),
              TextFormField(controller: _instructionsController, decoration: _inputDecoration('Instructions'), maxLines: 5, validator: (val) => val == null || val.isEmpty ? 'Veuillez entrer les instructions' : null),
              const SizedBox(height: 20),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Ingrédients', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  IconButton(icon: const Icon(Icons.add_circle_outline), color: peach, onPressed: _addIngredientDialog),
                ],
              ),
              const SizedBox(height: 8),
              _ingredients.isEmpty
                  ? const Text('Aucun ingrédient ajouté.', style: TextStyle(color: Colors.grey))
                  : Wrap(
                spacing: 8,
                runSpacing: 8,
                children: _ingredients
                    .map((ing) => Chip(
                  label: Text(ing['name'] ?? 'Ingrédient'),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _loading ? const CircularProgressIndicator(color: Colors.white) : Text(_isEditing ? 'Mettre à jour' : 'Ajouter la recette', style: const TextStyle(fontSize: 16)),
                ),
              ),
            ],
          ),
