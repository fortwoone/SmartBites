...
class _RecipeListPageState extends State<RecipeListPage> {
  ...
  Future<void> _deleteRecipe(Map<String, dynamic> recipe) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Êtes-vous sûr de vouloir supprimer cette recette ?'),
        actions: [
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
    appBar: AppBar(
    title: const Text('Mes Recettes', style: TextStyle(fontWeight: FontWeight.bold)),
    backgroundColor: Colors.deepOrangeAccent.shade100,
    elevation: 0,
    actions: [
    IconButton(
    icon: Icon(_showOnlyMine ? Icons.person : Icons.people_alt_outlined),
    tooltip: _showOnlyMine ? 'Afficher toutes les recettes' : 'Afficher mes recettes',
